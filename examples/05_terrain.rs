...
use glam::Vec3;
use walker2::testkit::{write_trace_svg, Checks, Runner, RollingGround, SlopeGround};
...
fn run() -> i32 {
    ...
    // Rolling hills.
    let rolling = RollingGround { amplitude: 0.6, wavelength: 9.0 };
    let mut walker = Walker::new(spec, &rolling);
    let mut runner = Runner::new(1.0 / 60.0);
    runner.run(&mut walker, &rolling, 12.0, |_, _| WalkerCommand::walk(Vec3::Z, 0.0));
    let dist = walker.position().z;
    checks.check_ge("rolling: distance (12s walk)", dist, spec.speed.walk * 12.0 * 0.6);
    checks.check_le("rolling: max tilt (rad)", runner.max_tilt(), 0.30);
    let v = walker.validation();
    checks.check_le("rolling: same-foot double steps", v.total_same_foot as f32, 2.0);
    checks.check_le("rolling: reach violations", v.total_reach_violations as f32, 400.0);
    println!("  rolling: {dist:.1}m, steps={}, reach_violations={}", v.total_steps, v.total_reach_violations);
    runner.print_reports();
    let _ = write_trace_svg("target/traces/05_terrain_rolling.svg", &runner, "05 rolling hills walk");

    // Constant climb.
    let slope = SlopeGround { grade: 0.14 };
    let mut climber = Walker::new(spec, &slope);
    let mut climb_runner = Runner::new(1.0 / 60.0);
    climb_runner.run(&mut climber, &slope, 10.0, |_, _| WalkerCommand::sprint(Vec3::Z, 0.0));
    let climb_dist = climber.position().z;
    let climb_height = climber.position().y - slope.height_at(0.0, 0.0);
    checks.check_ge("slope: distance (10s sprint, 14% grade)", climb_dist, spec.speed.sprint * 10.0 * 0.5);
    checks.check_le("slope: max tilt (rad)", climb_runner.max_tilt(), 0.30);
    let vc = climber.validation();
    checks.check_le("slope: same-foot double steps", vc.total_same_foot as f32, 2.0);
    println!(
